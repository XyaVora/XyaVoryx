...
function resolveBugbotProfileName(profileValue) {
  const normalized = typeof profileValue === "string" ? profileValue.trim().toLowerCase() : "";
...
const enforcementPath = process.env.BUGBOT_ENFORCEMENT_PATH || ".github/bugbot-enforcement.json";
const profilePath = process.env.BUGBOT_PROFILE_PATH || ".github/bugbot-profile.json";
const repositoryProfilePath = process.env.BUGBOT_REPOSITORY_PROFILE_PATH || ".github/bugbot-repository-profile.json";
const rules = loadRules(rulesPath);
const enforcementPolicy = loadOptionalJson(enforcementPath);
const profileConfig = loadOptionalJson(profilePath);
const repositoryProfile = loadOptionalJson(repositoryProfilePath);
const projectTypeProfileMap = {
...
const projectTypeProfile = typeof repositoryProfile.projectType === "string"
  ? projectTypeProfileMap[String(repositoryProfile.projectType).trim().toLowerCase()] ?? "balanced"
  : "balanced";
const activeProfile = resolveBugbotProfileName(
  process.env.BUGBOT_PROFILE ||
  profileConfig.profile ||
  repositoryProfile.deterministicProfile ||
  projectTypeProfile
);
const profilePreset = profilePresets[activeProfile] ?? profilePresets.balanced;
...
const mediumThresholdGateDefaults =
  rules.qualityGates?.mediumFindingsThreshold ?? defaultRules.qualityGates.mediumFindingsThreshold;
const profileMediumThreshold = profilePreset.mediumFindingsThreshold ?? {};
const mediumThresholdPolicy = enforcementPolicy.mediumFindingsThreshold ?? {};
const mediumThresholdGate = {
  enabled:
    typeof mediumThresholdPolicy.enabled === "boolean"
      ? mediumThresholdPolicy.enabled
      : typeof profileMediumThreshold.enabled === "boolean"
        ? profileMediumThreshold.enabled
        : mediumThresholdGateDefaults.enabled,
  blockAtTotal:
    typeof mediumThresholdPolicy.blockAtTotal === "number"
      ? mediumThresholdPolicy.blockAtTotal
      : typeof profileMediumThreshold.blockAtTotal === "number"
        ? profileMediumThreshold.blockAtTotal
        : mediumThresholdGateDefaults.blockAtTotal,
  blockAtSecurity:
    typeof mediumThresholdPolicy.blockAtSecurity === "number"
      ? mediumThresholdPolicy.blockAtSecurity
      : typeof profileMediumThreshold.blockAtSecurity === "number"
        ? profileMediumThreshold.blockAtSecurity
      : mediumThresholdGateDefaults.blockAtSecurity
};
...
const defaultEnforcement = rules.enforcement ?? defaultRules.enforcement;
const profileEnforcement = profilePreset.enforcement ?? {};
const enforcement = {
  blockSeverities: Array.isArray(enforcementPolicy.blockSeverities)
    ? enforcementPolicy.blockSeverities
    : Array.isArray(profileEnforcement.blockSeverities)
      ? profileEnforcement.blockSeverities
      : defaultEnforcement.blockSeverities ?? defaultRules.enforcement.blockSeverities,
  blockOnGateHits: typeof enforcementPolicy.blockOnGateHits === "boolean"
    ? enforcementPolicy.blockOnGateHits
    : typeof profileEnforcement.blockOnGateHits === "boolean"
      ? profileEnforcement.blockOnGateHits
    : typeof defaultEnforcement.blockOnGateHits === "boolean"
      ? defaultEnforcement.blockOnGateHits
      : defaultRules.enforcement.blockOnGateHits
};
...
const blockSeverities = new Set(
  (Array.isArray(enforcement.blockSeverities) ? enforcement.blockSeverities : defaultRules.enforcement.blockSeverities).map((value) =>
    String(value).toLowerCase()
  )
);
const blockedBySeverity = allFindings.filter((finding) => blockSeverities.has(String(finding.severity).toLowerCase()));
const blockedByGateHits = enforcement.blockOnGateHits ? gateHits.flatMap((entry) => entry.matches) : [];
const status = blockedBySeverity.length > 0 || blockedByGateHits.length > 0 ? "blocked" : "pass";
...
lines.push(`- Enforcement block severities: ${Array.from(blockSeverities).join(", ") || "none"}`);
lines.push(`- Enforcement gate blocking: ${enforcement.blockOnGateHits ? "enabled" : "disabled"}`);
lines.push("");
